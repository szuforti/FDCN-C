...
from preprocessing.BCI_CI_2A import raw
from collections import Counter
import matplotlib.pyplot as plt
from sklearn.utils import check_random_state

CONSTANT = CONSTANT['BCI_CI_2A']
...
label_name = ["left", "right", "Foot", "Tongue"]  # 标签
...
def load_BCIC2a(PATH, subjects, new_smp_freq, num_class, id_chosen_chs, start=0, stop=4):
...
class sour_data_with_FastICA():
    def __init__(self,
        ...
        self.data_name = 'BCI_CI_2A'

        X_train_all, y_train_all, X_test_all, y_test_all = load_BCIC2a(source_list, subjects, freq,
                                                                       num_class,
                                                                       id_chosen_chs, start=start_time,
...
if __name__ == '__main__':
    # gc.enable()
    use_time = time.time()
    data_list = r"D:\BCI_DATA\BCI_CI_2A\raw"
    for subject_index in range(1, 9):
        subject = [subject_index]
        Source_data = BCI_CI_2A_dataset(data_list=data_list, subjects=subject, start_time=0.0, time=4,
                                        sel_chs=None,
